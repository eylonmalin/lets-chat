...
var pipeline = [
    ...
    function k8sTrainingDbUrl(context) {
        console.log("Going to replace DB URI for K8s Training")
        if (process.env.MONGO_USER) {
            context.result.database.uri = "mongodb://"+process.env.MONGO_USER+":"+process.env.MONGO_PASS+"@"+process.env.MONGO_HOST+":"+process.env.MONGO_PORT;
        }else if (process.env.MONGO_HOST) {
            context.result.database.uri = "mongodb://"+process.env.MONGO_HOST+":"+process.env.MONGO_PORT+"/letschat";
        }
